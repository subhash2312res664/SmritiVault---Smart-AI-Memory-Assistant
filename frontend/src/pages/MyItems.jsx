      ...
      <div className="page">
        <div className="container">
          ...
          {loading ? (
            ...
            <div style={s.empty}>
              <p style={{ color: '#9ca3af', marginBottom: 16 }}>{search ? 'No items match your search.' : 'No items yet.'}</p>
            ...
            <div style={s.grid}>
              {paginated.map((item, i) => (
                <div key={i} className="card" style={s.card}>
                  ...
                  <p style={s.itemDesc}>{item.log_type || 'manual'} item</p>
                  <div style={s.itemLoc}>
                  ...
                  <div style={s.cardFooter}>
                    <span className="tag" style={{ fontSize: 10 }}>{(item.log_type || 'MANUAL').toUpperCase()}</span>
          ...
          {totalPages > 1 && (
            <div style={s.pagination}>
              <button className="btn-outline" style={{ padding: '8px 14px' }} onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}>
                ‹
              </button>
              <span style={{ fontSize: 14, color: '#6b7280' }}>Page {page} of {totalPages}</span>
              <button className="btn-outline" style={{ padding: '8px 14px' }} onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}>
                ›
              </button>
            </div>
          )}
...
const s = {
  ...
  menuBtn:     { background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', padding: '4px 6px', borderRadius: 6, transition: 'all 0.15s' },
